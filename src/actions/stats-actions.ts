...
export async function getStats(): Promise<StatsData> {
  ...
  const preferredMode = (
    ...
    (best, [mode, data]) => (data.count > best.count ? { mode, count: data.count } : best),
    { mode: "short" as FocusMode, count: 0 }
  ).mode;

  // Daily focus for last 30 days + streak calculation
  const daysWithSessions = new Map<string, { totalMinutes: number; sessionCount: number }>();

  for (const s of completed) {
    const key = formatDateKey(s.startedAt);
    const existing = daysWithSessions.get(key) ?? { totalMinutes: 0, sessionCount: 0 };
  ...
  const projectMap = new Map<
    string,
    { name: string; color: string; totalSeconds: number; count: number; lastDate: Date | null }
  >();
  for (const s of completed) {
