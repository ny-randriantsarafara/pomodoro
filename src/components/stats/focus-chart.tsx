...
function CustomTooltip({
  ...
  return (
    <div className="rounded-lg border border-[#1F1F23] bg-[#141416] px-3 py-2 shadow-lg">
      ...
      <p className="text-sm font-medium text-[var(--accent)]">{formatted}</p>
    ...
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        ...
        <YAxis
          ...
          axisLine={{ stroke: "#1F1F23" }}
          tickLine={{ stroke: "#1F1F23" }}
          label={{ value: "min", position: "insideTopRight", fill: "#71717A" }}
        />
        <Tooltip content={<CustomTooltip />} cursor={{ fill: "rgba(160,160,255,0.1)" }} />
        <Bar
          dataKey="totalMinutes"
          fill="#A0A0FF"
          radius={[4, 4, 0, 0]}
